...
OUT_DIR="$PROJECT_DIR/output/px4_offboard_repeated"
...
start_sitl() {
    ...
    (
        cd "$PX4_DIR"
        HEADLESS=1 make px4_sitl gz_x500 > /dev/null 2> "$LOG_DIR/${case_name}_sitl.err"
    ) &
...
for repeat in $(seq 1 "$REPEATS"); do
    ...
    run_case "${prefix}_single_obstacle_baseline_no_cbf" "single_obstacle" "baseline_no_cbf" \
        --disable-cbf --goal-y 2.0 --speed 0.35 --safe-radius 1.15 --alpha 0.7 --margin 0.05
    run_case "${prefix}_single_obstacle_cbf_velocity_filter" "single_obstacle" "cbf_velocity_filter" \
        --goal-y 2.0 --speed 0.35 --safe-radius 1.15 --alpha 0.7 --margin 0.05

    run_case "${prefix}_large_obstacle_baseline_no_cbf" "large_obstacle" "baseline_no_cbf" \
        --disable-cbf --goal-y 2.0 --speed 0.35 --safe-radius 1.35 --alpha 0.7 --margin 0.05
    run_case "${prefix}_large_obstacle_cbf_velocity_filter" "large_obstacle" "cbf_velocity_filter" \
        --goal-y 2.0 --speed 0.30 --max-speed 0.45 --safe-radius 1.35 --alpha 0.7 --margin 0.20

    run_case "${prefix}_multi_obstacle_baseline_no_cbf" "multi_obstacle" "baseline_no_cbf" \
        --disable-cbf --goal-y 1.4 --speed 0.35 --obstacles "2.6,-0.45,0.80;3.7,0.55,0.80" --alpha 0.7 --margin 0.05
    run_case "${prefix}_multi_obstacle_cbf_velocity_filter" "multi_obstacle" "cbf_velocity_filter" \
        --goal-y 1.4 --speed 0.35 --obstacles "2.6,-0.45,0.80;3.7,0.55,0.80" --alpha 0.7 --margin 0.05

    run_case "${prefix}_narrow_passage_baseline_no_cbf" "narrow_passage" "baseline_no_cbf" \
        --disable-cbf --goal-y 0.0 --speed 0.32 --obstacles "3.0,-0.95,0.72;3.0,0.95,0.72" --alpha 0.65 --margin 0.04
    run_case "${prefix}_narrow_passage_cbf_velocity_filter" "narrow_passage" "cbf_velocity_filter" \
        --goal-y 0.0 --speed 0.32 --obstacles "3.0,-0.95,0.72;3.0,0.95,0.72" --alpha 0.65 --margin 0.04

    run_case "${prefix}_wind_bias_baseline_no_cbf" "wind_bias" "baseline_no_cbf" \
        --disable-cbf --goal-y 2.0 --speed 0.35 --safe-radius 1.15 --alpha 0.7 --margin 0.05 --wind-vx 0.10 --wind-vy -0.18
    run_case "${prefix}_wind_bias_cbf_velocity_filter" "wind_bias" "cbf_velocity_filter" \
...
echo "Repeated PX4/Gazebo results written to $OUT_DIR"
